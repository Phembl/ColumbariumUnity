...
public class StoryManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject[] chapterHolder;
   ...
   private void SetupInputActions()
    ...
    private IEnumerator ResetWorld()
    {
        ...
        for (int i = 0; i < chapterHolder.Length; i++)
        {
            if (chapterHolder[i] != null)
            {
                
                GameObject storypointHolder = chapterHolder[i].transform.Find("Storypoints").gameObject;
                
                if (storypointHolder != null)
                {
                    for (int j = 0; j < storypointHolder.transform.childCount; j++)
                    {
                        StoryPoint nextStorypoint = storypointHolder.transform.GetChild(j).GetComponent<StoryPoint>();
                        nextStorypoint.Reset();
                    }
                }
                
                chapterHolder[i].SetActive(false);
    ...
    private void SwitchChapter(int chapter, bool newPlayerPosition)
    {
        ...
        chapterHolder[chapter].SetActive(true);
        ...
        if  (newPlayerPosition)
        {
            ...
            Transform playerStart = chapterHolder[chapter].transform.Find("PlayerStart");
        ...
        GameObject storypointHolder = chapterHolder[chapter].transform.Find("Storypoints").gameObject;
        ...
        if (storypointHolder != null)
        {
            for (int j = 0; j < storypointHolder.transform.childCount; j++)
            {
                ...
                {
                    nextStorypoint.FadeIn();
                }
                else
                {
                    // Invert Color for nein Garten invert
                    nextStorypoint.FadeIn(true);
                }
    ...
    public void StoryPointTriggered(GameObject currentStoryPoint, bool triggerOnly, bool firstTime = true)
    {
        if (!triggerOnly)
        {
            StoryPoint storyPoint = currentStoryPoint.GetComponent<StoryPoint>();
            AudioClip audioClip = storyPoint.storyAudioClip;
            Vector3 position = storyPoint.transform.position;
