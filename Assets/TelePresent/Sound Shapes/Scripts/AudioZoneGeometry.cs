...
public static class AudioZoneGeometry
{
    ...
    public static Vector3 GetConstrainedPosition(Vector3 target, List<Vector3> points, bool closedLoop, Transform transform)
    {
        ...
        List<Vector3> worldPoints = new(points.Count);
        foreach (Vector3 localPt in points)
    ...
    /// </summary>
    private static Vector3 GetClosestPointOnMesh(Vector3 target, MeshFilter meshFilter, AudioZone zone, SoundShapes_CachedMeshData cachedData = null)
    {
        Vector3[] vertices;
        int[]     triangles;
        ...
        if (audioZone.meshFilters != null)
        {
            foreach (MeshFilter mf in audioZone.meshFilters)
            {
                if (mf && mf.sharedMesh)
                {
                    ...
                    SoundShapes_CachedMeshData cmd = new()
                    {
                        meshReference = mesh,
    ...
    public static Vector3 GetClosestPointOnMeshes(Vector3 target, List<MeshFilter> meshFilters, AudioZone zone, List<SoundShapes_CachedMeshData> cachedMeshes = null)
    {
        ...
        foreach (MeshFilter mf in meshFilters)
        {
            ...
            SoundShapes_CachedMeshData cachedData = cachedMeshes?.Find(x => x.meshReference == mf.sharedMesh);
    ...
    public static Vector3 ClosestPointOnTriangle(Vector3 a, Vector3 b, Vector3 c, Vector3 p)
    {
        ...
        if (distTemp < minDist) closest = temp;
        return closest;
    }
    ...
    public static List<Vector3> GetOffsetPerimeter(AudioZone audioZone)
    {
        ...
        float         triggerDistance = (audioZone.triggerDistanceOverride > 0f)
            ...
            : (audioZone.audioSource ? audioZone.audioSource.maxDistance : 0f);

        if (audioZone.flipTriggerDistance)
            triggerDistance = -triggerDistance;

        if (audioZone.mode == AudioZone.ZoneMode.Shape &&
            audioZone.closedShape                   &&
            audioZone.points.Count >= 3)
        {
            List<Vector3> worldPoints = new(audioZone.points.Count);
            ...
            for (int i = 0; i < worldPoints.Count; i++)
            {
                ...
                Vector3 normal  = new(-avgDir.z, 0, avgDir.x);
        ...
        else if (audioZone.mode == AudioZone.ZoneMode.Shape && audioZone.points.Count >= 2)
        {
            ...
            List<Vector3> offsetUpper = new();
            List<Vector3> offsetLower = new();
            for (int i = 0; i < worldPoints.Count; i++)
            {
                Vector3 tangent;
                if (i == 0)
                    ...
                    tangent = ((worldPoints[i + 1] - worldPoints[i]) +
                               (worldPoints[i] - worldPoints[i - 1])).normalized;
